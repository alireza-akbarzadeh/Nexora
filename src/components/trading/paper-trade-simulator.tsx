...
import { Sparkline } from "@/components/landing/shared/sparkline"
...
type PaperTradeSimulatorProps = {
  className?: string
  /** Fire notify toasts on fill / error (dashboard). Landing keeps inline flash. */
  useNotify?: boolean
}
...
export function PaperTradeSimulator({
  ...
  const [priceHistory, setPriceHistory] = useState<Record<string, number[]>>(
    () =>
      Object.fromEntries(
        INITIAL_MARKETS.map((m) => [m.sym, Array(40).fill(m.price)]),
      ),
  )
  ...
  function flashMsg(kind: FlashMessage["kind"], msg: string) {
    setFlash({ kind, msg })
    if (!useNotify) return
    if (kind === "err") notify.error(msg)
    else if (!msg.includes("Bought") && !msg.includes("Sold")) {
      notify.success(msg)
    }
  }
  ...
  function execute() {
    ...
    if (!amountUsd || amountUsd <= 0) {
      flashMsg("err", "Enter an amount greater than 0")
      return
    }
    ...
    if (side === "buy") {
      if (amountUsd > cash) {
        flashMsg("err", "Insufficient cash balance")
        return
      }
      ...
      setHoldings((h) => {
        ...
        return {
          ...
          [market.sym]: {
            qty: cur.qty + qty,
            costBasis: cur.costBasis + amountUsd,
          },
        }
      })
      const msg = `Bought ${qty.toFixed(6)} ${market.sym} @ ${fmtUSD(market.price)}`
      setFlash({ kind: "ok", msg })
      if (useNotify) {
        notify.order({
          side: "buy",
          symbol: `${market.sym}/USD`,
    ...
    if (!cur || cur.qty * market.price < amountUsd - 0.01) {
      flashMsg("err", `Not enough ${market.sym} to sell`)
      return
    }
    ...
    const msg = `Sold ${sellQty.toFixed(6)} ${market.sym} @ ${fmtUSD(market.price)}`
    setFlash({ kind: "ok", msg })
    if (useNotify) {
      notify.order({
        side: "sell",
        symbol: `${market.sym}/USD`,
        amount: sellQty.toFixed(6),
        type: "market",
        price: market.price,
        status: "filled",
      })
  ...
  function reset() {
    ...
    setHoldings({})
    flashMsg("ok", "Demo portfolio reset")
  }

  // Avoid double toasts when useNotify: skip inline flash for ok/err shown via notify
  const showInlineFlash = flash && !useNotify
  ...
  return (
    <div className={cn("grid gap-6 lg:grid-cols-5", className)}>
      <div className="card-elevated rounded-3xl p-6 lg:col-span-2">
          ...
          <div className="flex items-baseline justify-between">
            <div>
              ...
              <div
                ...
                style={{
                  color: tickUp ? "var(--emerald-glow)" : "var(--loss)",
                }}
              >
                {fmtUSD(market.price)}
              </div>
        ...
        >
          {side === "buy" ? `Buy ${market.sym}` : `Sell ${market.sym}`} ·{" "}
          {fmtUSD(amountUsd)}
        </button>
      ...
      <div className="space-y-6 lg:col-span-3">
        <div className="grid grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            accent={
              totalPnL >= 0 ? "var(--emerald-glow)" : "var(--loss)"
            }
          />
          <StatCard
          ...
          {positions.length === 0 ? (
            <div className="py-14 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No open positions yet. Place your first simulated order to see
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          color: up ? "var(--emerald-glow)" : "var(--loss)",
                        }}
                      >
                        {up ? (
